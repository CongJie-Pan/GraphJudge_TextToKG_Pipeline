"""
LoRA Fine-tuning for Graph Judgment on GenWiki Dataset

This script implements Parameter-Efficient Fine-Tuning (PEFT) using Low-Rank Adaptation (LoRA)
for fine-tuning LLaMA-3-8B-Instruct model on graph judgment tasks using the GenWiki dataset.

Main functionality:
1. Loads instruction-response dataset for graph judgment training
2. Applies LoRA adapters to reduce trainable parameters while maintaining performance
3. Fine-tunes the model to judge correctness of knowledge graph triples
4. Supports distributed training and efficient memory management
5. Implements custom data preprocessing for instruction-following format

LoRA allows efficient fine-tuning by adding small trainable matrices to the model
while keeping the pre-trained weights frozen, significantly reducing memory requirements
and training time while achieving comparable performance to full fine-tuning.
"""

import os
os.environ["這˙˙CUDA_VISIBLE_DEVICES"] = "2"  # Set GPU device for training
import sys
import torch
...
# These parameters are optimized for RTX 4090. For larger GPUs, you can increase batch sizes

MICRO_BATCH_SIZE = 8                # Batch size per device (power of 2 for efficiency)
...
DATA_PATH = "data/genwiki_4omini_context/train_instructions_context_llama2_7b.json"  # Training data path
OUTPUT_DIR = "models/llama3-8b-instruct-lora-genwiki-context"  # Directory to save fine-tuned model
...
def generate_and_tokenize_prompt(data_point):
    """
    Process a single data point into tokenized format for training.
    ...
    fine-tuning. It creates a structured prompt and ensures that only the response
    portion is used for loss computation (input portion has labels set to -100).
    
    The prompt format follows the Alpaca convention:
    - Instruction: Description of the task
    - Input: Additional context (optional)
    - Response: Expected output for training
    """
