"""
LoRA Fine-tuned Model Batch Inference for Graph Judgment - Case GJ Configuration

This script performs efficient batch inference using a LoRA fine-tuned LLaMA model 
specifically configured for the "Case GJ" (Graph Judgment) variant. It's optimized 
for the SciERC dataset with context-aware processing.

Main functionality:
1. Loads a base LLaMA model with specialized LoRA adapters for SciERC dataset
2. Implements both simple and advanced prompt templates for graph judgment
3. Supports efficient batch processing with JSON input format
4. Handles variable-length sequences with proper padding and attention masks
5. Processes scientific literature-based graph judgment tasks

This variant is specifically tuned for scientific text understanding and relationship
extraction from academic papers, making it suitable for evaluating knowledge graphs
derived from scientific literature.
"""

import os
os.environ["CUDA_VISIBLE_DEVICES"] = "4"  # Set GPU device for inference
import sys
import torch
...
# LORA_WEIGHTS = "models/llama2-7b-lora-rebel-sub-context-tmp/"  # REBEL-sub with context
LORA_WEIGHTS = "models/llama2-7b-lora-scierc-context-0518"       # SciERC context model (default for Case GJ)
...
def generate_advanced_prompt(instruction, input=None):
    ...
    This function creates a more detailed prompt template specifically designed
    for scientific text understanding and relationship validation. It includes:
    - Clear task definition for graph judgment in scientific contexts
    - Specific attention points for grammatical and factual correctness
    - Example demonstrations with scientific content
    - Explicit binary output format requirements
    """
    prompt = f"""
Goal:
...
def batch_evaluate(
    ...
    **kwargs,
    ...
    This function provides the core batch processing capability, specifically
    optimized for scientific text understanding tasks. It handles variable-length 
    inputs through padding and attention masks while maintaining the quality of
    ...
    inputs = tokenizer(
        ...
        max_length=512              # Maximum sequence length (suitable for scientific texts)
    )
...
if __name__ == "__main__":
    ...
    # Set up argument parser for SciERC dataset configuration
    ...
    # Default configuration for SciERC dataset with JSON input format
    parser.add_argument("--finput", type=str, 
                       default="data/scierc_4omini_context/test_instructions_context_llama2_7b.json",
                       help="Input JSON file containing SciERC test instructions")
    parser.add_argument("--foutput", type=str, 
                       default="data/scierc_4omini_context/pred_instructions_context_llama2_7b_gj.csv",
                       help="Output CSV file for Graph Judgment predictions")
    ...
    # Load input data from JSON file using HuggingFace datasets
    # This variant uses JSON format instead of CSV for richer data structure
    ...
    batch_size = 16  # Smaller batch size for scientific texts (adjustable based on GPU memory)
    
    # Extract instruction prompts from the dataset
    prompts = data_eval['instruction']
    
    # Process data in batches for efficient inference
    for i in tqdm(range(0, total_num, batch_size), desc="Processing SciERC batches"):
    ...
    print(f"SciERC Graph Judgment inference completed! Results saved to: {args.foutput}")
