"""
LoRA Fine-tuning for Graph Judgment on SciERC Dataset with Context

This script implements Parameter-Efficient Fine-Tuning (PEFT) using Low-Rank Adaptation (LoRA)
for fine-tuning LLaMA-3-8B-Instruct model on graph judgment tasks using the SciERC dataset.
The SciERC dataset focuses on scientific literature understanding and entity relationships
in academic papers.

Main functionality:
1. Loads instruction-response dataset specifically designed for scientific text understanding
2. Applies LoRA adapters for efficient fine-tuning on scientific domain knowledge
3. Fine-tunes the model to judge correctness of scientific knowledge graph triples
4. Supports distributed training with specialized hyperparameters for scientific texts
5. Implements custom data preprocessing for scientific instruction-following format

This variant is optimized for understanding relationships in scientific literature,
making it particularly effective for knowledge graphs derived from academic papers,
research publications, and scientific documentation.
"""

import os
os.environ["CUDA_VISIBLE_DEVICES"] = "3"  # Set GPU device for training
import sys
import torch
...
# These parameters are optimized for scientific text understanding and longer sequences

MICRO_BATCH_SIZE = 4                # Smaller batch size due to longer scientific texts
...
EPOCHS = 12                         # More epochs for scientific domain adaptation
STEPS = 500                         # Maximum training steps (overrides epochs)
...
DATA_PATH = "data/scierc_4omini_context/train_instructions_context_llama2_7b.json"  # SciERC training data path
OUTPUT_DIR = "models/llama3-8b-instruct-lora-scierc-context"  # Directory to save fine-tuned model
...
def generate_and_tokenize_prompt(data_point):
    """
    Process a single data point into tokenized format for training on scientific texts.
    ...
    fine-tuning, specifically adapted for scientific text understanding. It handles
    longer sequences common in scientific literature and ensures proper masking
    for scientific relationship evaluation training.
    
    The prompt format is optimized for scientific domain understanding:
    - Instruction: Description of the scientific relationship evaluation task
    - Input: Scientific context or additional information (optional)
    - Response: Expected judgment for training
    """
    ...
    if input_text:
        # Format with both instruction and scientific context
...
if __name__ == "__main__":
    ...
    # Load the SciERC training dataset from JSON file
